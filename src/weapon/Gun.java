...
public class Gun extends GameObjForAnimator {
    ...
    public enum GunType {
        ...
        UZI("/pictures/weapon/uzi.png", "/pictures/actor/uzi3.png","/sounds/weapon/ak.wav","/sounds/weapon/UZIR.wav",70, 54,29,44, 500, 40, 7, 60, 0,5),
        AK("/pictures/weapon/ak.png", "/pictures/actor/ak3.png","/sounds/weapon/ak-2.wav","/sounds/weapon/AKR.wav",70, 54,53,56, 650, 30, 10, 90, 0,3),
        SNIPER("/pictures/weapon/sniper.png", "/pictures/actor/sniper3.png","/sounds/weapon/sniper.wav","/sounds/weapon/sniperR.wav",70, 54,66,60, 70, 10, 60, 100, 0,2),
        MACHINE_GUN("/pictures/weapon/machine.png", "/pictures/actor/machine3.png","/sounds/weapon/ak.wav","/sounds/weapon/machineGunR.wav",70, 54,62,62, 800, 100, 5, 150, 0,2);


        public String forMapPath;
        public String forActorPath;
        public String shootSoundsPath;
        ...
        GunType(String forMapPath, String forActorPath, String shootSoundsPath,String reloadSoundPath, int width, int height, int widthForActor, int heightForActor, int maxMagazine, int magazine, int shootingDelay, int reloadingDelay, int beginShoot, int moveSpeed) {
            this.forMapPath = forMapPath;
            this.forActorPath = forActorPath;
            this.shootSoundsPath = shootSoundsPath;
    ...
    public boolean shoot() {
        if (canShoot) {
            if (shootingDelay.isStop() || (getGunType() == GunType.PISTOL&&aloneClick.isPause())) {
                ...
                shootingDelay.play();
                if (magazine <= 0) {
                    AudioResourceController.getInstance().shot("/sounds/weapon/button.wav");
                    return false;
                }
                AudioResourceController.getInstance().shot(gunType.shootSoundsPath);
                magazine--;
                return true;
            }
        }
    ...
    public void reloading() {//要修正裝彈延遲
        ...
        if (canReloading) {
            reloadingDelay.play();
            AudioResourceController.getInstance().play(gunType.reloadSoundPath);
